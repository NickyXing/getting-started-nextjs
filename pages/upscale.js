...
export default function Home() {
  ...
  const removeBg = async () => {
    ...
    const response = await fetch("/api/upscale", {
      ...
      body: JSON.stringify({
        input: filePath,
        modelType: "general_scenes", // faces or general_scenes
        resolution: resolution, // 1-4放大倍率
      }),
    });
    ...
    while (
      ...
      await sleep(1000);
      const response = await fetch("/api/upscale/" + removeBgOutputs.id);
      ...
      if (removeBgOutputs.status === "succeeded") {
        // 转存到我的图片库
        // 插入图片记录
        ...
        // 插入图片记录
        await fetch("/api/add-img-record/upscale", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            authorization: localStorage.getItem("token"),
  ...
  const queryRemoveRecord = async () => {
    if (localStorage.getItem("token")) {
      const res = await fetch("/api/query-img-record/upscale", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          authorization: localStorage.getItem("token"),
  ...
  const download = async () => {
    try {
      ...
      // 设置下载文件名
      link.download = `upscale.${fileExtension}`;
            ...
            {(!removeBgOutputs ||
              (removeBgOutputs &&
                (!removeBgOutputs.output ||
                  removeBgOutputs.output.length === 0))) && (
              <div>
                <span className="mr-2 ">magnification: </span>
                <Select
                  defaultValue={2}
